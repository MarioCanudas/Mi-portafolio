...
class PostState(PortfolioState):
    ...
    def current_post(self) -> dict:
        ...
        for p in self.posts:
            if p["slug"] == slug:
                return p
        return {"title": "post no encontrado", "content": "este post no existe.", "date": ""}

def post() -> rx.Component:
    return layout(
        rx.vstack(
            rx.heading(PostState.current_post["title"], size="6", margin_bottom="2", color="var(--gray-12)"),
            rx.text(PostState.current_post["date"], color="var(--gray-9)", margin_bottom="4"),
            rx.box(
                rx.markdown(
